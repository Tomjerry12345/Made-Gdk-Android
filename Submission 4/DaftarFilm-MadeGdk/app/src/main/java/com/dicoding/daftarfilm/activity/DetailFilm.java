...
public class DetailFilm extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		Glide.with(this)
				.load("https://image.tmdb.org/t/p/w500" + photo)
				...
				if (isChecked) {
					...
					if (noSimilarItem) {
						...
						if (result > 0) {
							...
							Toast.makeText(DetailFilm.this, "succes", Toast.LENGTH_SHORT).show();
						} else {
							Toast.makeText(DetailFilm.this, "Failed", Toast.LENGTH_SHORT).show();
						}
					}
					...
					FavoriteHelper favoriteHelper = FavoriteHelper.getInstance(DetailFilm.this);
					list = favoriteHelper.getAllFilm();
					int id = 0;
					for (int i = 0; i < list.size(); i++) {
						if (list.get(i).getTitle().equalsIgnoreCase(title) && list.get(i).getOverview().equalsIgnoreCase(desc)) {
