				...
				if (isChecked) {
					...
					if (noSimilarItem) {
						...
						if (result > 0) {
							...
							Toast.makeText(DetailTv.this, "succes", Toast.LENGTH_SHORT).show();
						} else {
							Toast.makeText(DetailTv.this, "Failed", Toast.LENGTH_SHORT).show();
						}
					}
					...
					FavoriteHelper favoriteHelper = FavoriteHelper.getInstance(DetailTv.this);
					list = favoriteHelper.getAllTv();
					int id = 0;
					for (int i = 0; i < list.size(); i++) {
						if (list.get(i).getName().equalsIgnoreCase(title) && list.get(i).getOverview().equalsIgnoreCase(desc)) {
